...
1. 덧셈 연산자를 사용하여 모든 사원에 대해서 $300의 급여 인상을
계산한후 사원이름, 급여, 인상된 급여를 출력하세요. 
...
select ename 사원이름, salary 급여,salary*12+NVL(commission,0)+100 as 총연봉
from employee
order by 총연봉 desc;



3. 급여가 2000을 넘는 사원의 이름과 급여를 급여가 많은것 
부터 작은순으로 출력하세요. 
...
select ename 사원이름, eno 사원번호, dno 부서번호
from employee
where eno = 7788;

5. 급여가 2000에서 3000사이에 포함되지 않는 사원의 이름과
급여를 출력 하세요. 
...
6. 1981년 2월 20일부터 81년 5월 1일 사이의 
...
9. 1981년도 입사한 사원의 이름과 입사일을 출력 하시오 
( like 연산자와 와일드 카드 사용 : _ , % )
...
select ename 사원이름, manager 사수, job 업무
from employee
where manager is null;

11. 커밋션을 받을 수 있는 자격이 되는 
사원의 이름, 급여, 커미션을 출력하되 급여및 커밋션을 
...
select ename 이름, job 업무, salary 급여
from employee
