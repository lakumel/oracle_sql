...
select trunc(sysdate-to_date('19950714','YYYYMMDD'))as "살아온일수"
from dual;
...
select sum(salary),round(avg(salary),2)as 평균,max(salary), min(salary),dno,count(dno)
from employee
where dno not in(20)
group by dno
order by dno;
...
select sum(salary), round(avg(salary),2)as 평균,max(salary),min(salary),dno, count(*)
from employee
